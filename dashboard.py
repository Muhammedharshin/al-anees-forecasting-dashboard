...
# CONFIG
# ============================================================
API_BASE_URL = "https://al-anees-forecasting-api.onrender.com"
...
def call_api(endpoint, params=None):
    try:
        ...
        st.error(f"⚠️ Could not reach API at {API_BASE_URL}{endpoint}\n\nError: {e}")
        st.stop()

# ============================================================
# SIDEBAR NAVIGATION
# ============================================================
st.sidebar.title("📦 AL ANEES")
st.sidebar.caption("Demand Forecasting & Stockout Prevention")
screen = st.sidebar.radio(
    "Navigate",
    ["Overview", "Demand Forecast", "Stockout Risk Monitor", "Replenishment Planner"]
)
st.sidebar.divider()
...
if screen == "Overview":
    st.title("📊 Overview")
    st.caption("Live snapshot of inventory health and model performance")

    health = call_api("/health")
    repl = call_api("/api/replenishment")
    perf = call_api("/api/model-performance")

    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Total SKUs Monitored", health["total_skus_in_dataset"])
    col2.metric("SKUs Needing Order", repl["skus_needing_order"])
    col3.metric("Total Units to Order", f"{repl['total_units_to_order']:,}")
    col4.metric("Best Model WAPE", f"{perf['xgboost']['wape']*100:.2f}%", "XGBoost")

    st.divider()

    risk = call_api("/api/stockout-risk")
    summary = risk["summary"]

    st.subheader("Stockout Risk Breakdown")
    rc1, rc2 = st.columns([1, 2])

    with rc1:
        for level, color in [("RED", "🔴"), ("AMBER", "🟠"), ("GREEN", "🟢")]:
            count = summary.get(level, 0)
            st.markdown(f"### {color} {level}: {count} SKUs")
    ...
    with rc2:
        fig = px.pie(
            ...
            color_discrete_map={"RED": "#C0392B", "AMBER": "#D98C00", "GREEN": "#1E8E5A"},
            hole=0.5
        )
    ...
    mc1.metric("XGBoost WAPE", f"{perf['xgboost']['wape']*100:.2f}%", "Primary model — all 200 SKUs")
    mc2.metric("Ensemble WAPE", f"{perf['ensemble']['wape']*100:.2f}%", "XGBoost + Prophet blend")
    mc3.metric("Prophet WAPE", f"{perf['prophet']['wape']*100:.2f}%", "Top 30 high-volume SKUs")
